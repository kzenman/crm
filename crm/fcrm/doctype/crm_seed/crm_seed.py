...
class CRMSeed(Document):
	...
	def set_lead_name(self):
		if not self.lead_name:
			...
			if not self.organization and not self.email and not self.flags.ignore_mandatory:
				frappe.throw(_("A Seed requires either a person's name or an organization's name"))
			elif self.organization:
				self.lead_name = self.organization
			elif self.email:
				self.lead_name = self.email.split("@")[0]
			else:
				self.lead_name = "Unnamed Seed"
	...
	def validate_email(self):
		if self.email:
			...
			if self.email == self.lead_owner:
				frappe.throw(_("Seed Owner cannot be same as the Seed Email Address"))
	...
	def assign_agent(self, agent):
		...
		assign({"assign_to": [agent], "doctype": "CRM Seed", "name": self.name})
...
def convert_to_deal(lead, doc=None, deal=None, existing_contact=None, existing_organization=None):
	if not (doc and doc.flags.get("ignore_permissions")) and not frappe.has_permission(
		...
		frappe.throw(_("Not allowed to convert Seed to Deal"), frappe.PermissionError)

	lead = frappe.get_cached_doc("CRM Seed", lead)
	if frappe.db.exists("CRM Seed Status", "Qualified"):
