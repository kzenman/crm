...
const orderMap = Object.fromEntries(
  GROUP_ORDER.map((s, i) => [normalizeKey(s), i])
);
...
const statusOf = (r) => {
  ...
  if (s && typeof s === 'object') return s.value ?? s.label ?? '';
  return '';
};
...
const sortedRows = computed(() => {
  ...
  src.sort((a, b) => {
    const ai = orderMap[norm(statusOf(a.r))] ?? 999;
    const bi = orderMap[norm(statusOf(b.r))] ?? 999;
    if (ai !== bi) return ai - bi;   // your fixed order
